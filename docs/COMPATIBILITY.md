...
| Vistas | Proyección, filtro, joins, agregación, orden, límite, desplazamiento y operaciones de conjuntos (`UNION`, `UNION ALL`, `INTERSECT`, `EXCEPT`) | `INNER`, `LEFT`, `CROSS`, `GROUP BY`, `HAVING`, agregados comunes y compounds de conjuntos con precedencia igual entre motores | Subconsultas, ventanas, CTE, y cadenas de compound que mezclan `INTERSECT` con `UNION`/`EXCEPT` (precedencia divergente entre motores) |
